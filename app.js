...
class App {
  ...
  renderStudentDashboard(panel) {
    ...
    const isOverdueHomework = dominantAssignment?.status === 'Overdue';
    const isHomework = !hasActiveTestPrep && dominantAssignment;
    
    const bannerIcon = hasActiveTestPrep ? '📌' : isHomework ? (isOverdueHomework ? '⚠️' : '📋') : '🎯';
    ...
    const paperHtml = visibleUnits.map((unit) => `
      <div class="student-open-paper-section" style="margin-bottom: 32px;">
        ...
        <div class="student-topic-grid-header" style="display: grid; grid-template-columns: 55px minmax(180px, 1fr) 110px 130px 110px 75px 220px; gap: 12px; padding: 8px 16px; font-size: 12px; font-weight: 700; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 8px;">
          ...
          <div>Confidence</div>
          <div>Mastery</div>
          <div style="text-align: right;">Last Checked</div>
  ...
  getObjectiveDiagramSvg(strandId) {
    ...
    if (id.includes('1.6.1') || id === '1.6.1') {
      return `
        <div class="essay-mastery-hub" style="margin: 20px 0; font-family: Inter, sans-serif;">
          <!-- Header & Pill Tabs (Clean, unboxed) -->
          <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px; flex-wrap: wrap; gap: 10px;">
            <h4 style="color: var(--teal); font-size: 15px; margin: 0; font-weight: 700;">📝 OCR 8-Mark Extended Response Essay Mastery Hub</h4>
            <div style="display: flex; gap: 6px; flex-wrap: wrap;">
              <button type="button" class="btn ${(!this.essayHubTab || this.essayHubTab === 'blueprint') ? 'btn-primary' : 'btn-secondary'} essay-hub-tab-btn" data-tab="blueprint" style="font-size: 12px; font-weight: 600; padding: 6px 14px; border-radius: 20px;">🏆 Level 3 Recipe</button>
              <button type="button" class="btn ${this.essayHubTab === 'model' ? 'btn-primary' : 'btn-secondary'} essay-hub-tab-btn" data-tab="model" style="font-size: 12px; font-weight: 600; padding: 6px 14px; border-radius: 20px;">✍️ Model Essay + Annotations</button>
              <button type="button" class="btn ${this.essayHubTab === 'pitfalls' ? 'btn-primary' : 'btn-secondary'} essay-hub-tab-btn" data-tab="pitfalls" style="font-size: 12px; font-weight: 600; padding: 6px 14px; border-radius: 20px;">⚠️ Examiner Pitfalls</button>
            </div>
          </div>

          <!-- TAB 1: BLUEPRINT & IMPACT GRID -->
          ${(!this.essayHubTab || this.essayHubTab === 'blueprint') ? `
            <div style="border-left: 4px solid var(--teal); padding: 8px 0 8px 16px; margin-bottom: 18px; background: rgba(45, 156, 145, 0.05); border-radius: 0 8px 8px 0;">
              <strong style="color: var(--teal); font-size: 13.5px;">🏆 Examiner Level 3 Recipe (7–8 Marks):</strong>
              <p style="font-size: 12.5px; color: var(--text-main); margin: 4px 0 0 0; line-height: 1.5;">
                1. <strong>Identify Stakeholders</strong> (pupils, low-income families, IT staff) &middot; 
                2. <strong>Dual Perspective</strong> (+ Benefits vs - Harms) &middot; 
                3. <strong>Scenario Context</strong> (reference prompt details) &middot; 
                4. <strong>Justified Conclusion</strong> (supported final trade-off judgment)
              </p>
            </div>

            <!-- 4 Impact Category Grid -->
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 14px;">
              <div style="border-top: 3px solid #10B981; padding-top: 10px;">
                <strong style="color: #10B981; font-size: 13.5px; display: block; margin-bottom: 6px;">🌳 Environmental Impacts</strong>
                <ul style="font-size: 12.5px; color: var(--text-main); padding-left: 16px; margin: 0; line-height: 1.5;">
                  <li><strong>E-Waste &amp; WEEE:</strong> Toxic heavy metals (lead/mercury) in discarded PCs.</li>
                  <li><strong>Energy Use:</strong> Data center electricity vs lower laptop wattage.</li>
                  <li><strong>Finite Resources:</strong> Rare earth mining for batteries &amp; chips.</li>
                </ul>
              </div>

              <div style="border-top: 3px solid #3B82F6; padding-top: 10px;">
                <strong style="color: #3B82F6; font-size: 13.5px; display: block; margin-bottom: 6px;">⚖️ Ethical &amp; Societal Impacts</strong>
                <ul style="font-size: 12.5px; color: var(--text-main); padding-left: 16px; margin: 0; line-height: 1.5;">
                  <li><strong>Digital Divide:</strong> Equal access for low-income pupils vs home broadband costs.</li>
                  <li><strong>Automation:</strong> Job displacement vs technical creation.</li>
                  <li><strong>Wellbeing:</strong> 24/7 connectivity &amp; screen fatigue.</li>
                </ul>
              </div>

              <div style="border-top: 3px solid #F59E0B; padding-top: 10px;">
                <strong style="color: #F59E0B; font-size: 13.5px; display: block; margin-bottom: 6px;">🌐 Cultural &amp; Community</strong>
                <ul style="font-size: 12.5px; color: var(--text-main); padding-left: 16px; margin: 0; line-height: 1.5;">
                  <li><strong>Globalisation:</strong> Instant global access vs local cultural loss.</li>
                  <li><strong>Flexible Work:</strong> Remote working benefits vs work-life boundary blur.</li>
                </ul>
              </div>

              <div style="border-top: 3px solid #EC4899; padding-top: 10px;">
                <strong style="color: #EC4899; font-size: 13.5px; display: block; margin-bottom: 6px;">🔒 Privacy &amp; Surveillance</strong>
                <ul style="font-size: 12.5px; color: var(--text-main); padding-left: 16px; margin: 0; line-height: 1.5;">
                  <li><strong>Data Collection:</strong> Tracking, facial recognition &amp; cookies.</li>
                  <li><strong>Cloud Breaches:</strong> Centralised cloud storage cybersecurity risks.</li>
                </ul>
              </div>
            </div>
          ` : ''}
          ...
          ${this.essayHubTab === 'pitfalls' ? `
            <div style="margin-top: 8px;">
              <h5 style="color: #EF4444; font-size: 14px; margin: 0 0 14px 0; font-weight: 700;">⚠️ 4 Mistakes That Lose Marks on the 8-Mark Essay:</h5>
