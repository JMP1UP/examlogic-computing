...
- **Student Navigation**: The student sidebar contains My desk, Topics, Practice, Progress and Messages. Legacy learning, recall, number, programming, simulator and exam routes remain executable, while their parent destination stays highlighted.
- **My Desk Route**: My desk is the pupil's personal Computing study space alongside school. It retains one recommended task and a bounded **Flashcards on your desk** view of at most three due-first topics, with any hidden count and an Organise route. Each topic shows cards ready and **Your card confidence**, derived from the pupil's latest distinct self-ratings and explicitly not an exam result.
- **Topics Route**: Topics presents the OCR specification by paper, topic and objective. A learner can add flashcards immediately for work met in class or independently open a refresher. Adding or pausing flashcards and opening an exactly mapped checked question do not change teacher controls or assessed evidence.
- **Focused Learn Route**: When a learner opens a specific objective, the page presents only its explanation, worked example and any contextual tool. The primary next action is an exactly mapped exam question; adding flashcards is independent and does not claim teaching or mastery. The recovery route returns to Topics.
- **Practice Hub**: Practice contains exactly four modes: Flashcards, Exam questions, Number skills and Programming. Each states its purpose, duration and evidence status while retaining the existing activity engines.
...
- **Weekly Notebook**: My desk shows a compact weekly notebook beneath the primary task: flashcards on two different calendar days, number-systems practice, programming and (on alternating weeks) one 4–6 mark exam-style answer. Required work remains primary and is not duplicated as an upcoming-test card. Each notebook task links directly to its activity; when the desk has no flashcard topics, that task links to topic selection instead of adding another empty panel. Detailed progress and desk information remains available in one collapsed disclosure. Only recorded learner activity satisfies notebook items; a class-wide assignment status is not treated as an individual completion record. A new week starts clean with no punitive streak or overdue optional backlog. Engagement status never changes mastery.
- **Test Preparation**: A teacher-set plan is organised by selected specification point. A point with a mapped exam-style task opens that exact question; a point without one opens its focused explanation and worked example. The plan does not label a short recall quiz as exam practice, and unavailable exam-question coverage is not invented.
- **Class Markbook**: Classes shows checked performance, learner-rated flashcard confidence, the latest teacher assessment report and work awaiting review for each authorised learner. Selecting a name opens a detailed profile. These sources remain separate and confidence cannot change attainment.
- **Assessment Reports**: After a selected-topic test, the teacher may record an optional overall mark, a stronger/developing/needs-review judgement for each included specification point and exam-technique tags. The learner sees direct links to matching teaching and exam-style practice. A report is teacher guidance, not a demonstrated StudySpice attempt.
...
- **Flashcards**: The learner's personal flashcard topics contain only safely mapped sections they explicitly added to their desk. Classroom and ambiguous legacy availability remain readable for compatibility but do not silently populate My desk. The learner writes an attempt, reveals the definition, then gives a required rating. The latest rating for each distinct card determines confidence and scheduling. Repeating one card cannot manufacture breadth or a top confidence label.
