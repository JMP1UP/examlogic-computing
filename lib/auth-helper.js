// Shared authentication helpers for serverless API routes.
...
const auth = {
  ...
  async validateSessionDb(decoded) {
    ...
    const { id, role, pwh, schoolId } = decoded;
    ...
    if (role === 'admin') {
      ...
      // New sessions always carry schoolId. The conditional keeps pre-deployment
      // sessions valid only until their existing 24-hour expiry.
      if (schoolId && admin.school_id !== schoolId) return null;
    ...
    if (role === 'coordinator' || role === 'teacher') {
      ...
      if (schoolId && coord.school_id !== schoolId) return null;
    ...
    if (role === 'student') {
      ...
      if (schoolId && student.school_id !== schoolId) return null;
